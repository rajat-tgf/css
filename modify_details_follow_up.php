...
$resultn = sqlsrv_query($conncss,"SELECT email_notification FROM follow_ups WHERE id = '$id_follow_up'");
...
$sql = "UPDATE follow_ups SET request = '$request', response = '$response', status = '$status', category = '$category', responsable = '$responsable', name = '$name', date_follow_up = '$date_follow_up', comments = '$comments', email_notification = '$email_notification' WHERE id = '$id_follow_up'";

$result = sqlsrv_query($conncss,$sql);
if($result){
	?>
<script>
	//parent.location.frames["side_menu"].location.reload();
	//refresh_menu();
