...
$result_member_allegation = sqlsrv_query($conncss,"SELECT member FROM intelligence_reports WHERE id = '$ir_id'"); 
$row_member_allegation = sqlsrv_fetch_array( $result_member_allegation );
$team_member = $row_member_allegation['member'];
...
$date_comments = date("Y-m-d");
	...
	$sql = "INSERT INTO comments_ir (ir_report, author, member, date_comment, comment, request_notification, read_notification ) VALUES ( '$ir_id', '$username', '$team_member', '$date_comments', '$comment', '$request_notification', '$read_notification' )";
...
if ( $request_notification == "yes" ){
	...
	$result_country = sqlsrv_query($conncss,"SELECT country FROM intelligence_reports WHERE id = '$ir_id'");
	...
	$itisofficer = ($officecount > 0);
		...
		if(!$itisofficer){
			...
			$subject  = "OIG CSS - Notification - Intelligence Report " . $ir_id . " - " . $country;
			
			$message  = "<u> Do not reply to this email. This email has been generated automatically </u><br><br>"; 
			$message1  = "A new comment in relation to Intelligence Reprot " . $ir_id . " - " . $country . " has been added for your attention.";
		...
		//SEND NOTIFICATION EMAIL TO MEMBER
			...
			$message  = "<u> Do not reply to this email. This email has been generated automatically </u><br><br>"; 
			$message1  = "A new comment in relation to Intelligence Report " . $ir_id . " - " . $country . " has been added for your attention.";
	...
	header("location: intelligence_report_details.php?new_comments='yes'");
}
