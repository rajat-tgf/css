...
if($result_sql1){
	...
	$row = sqlsrv_fetch_array($result);
	$profile_id = $row['id'];
	//$sql = "INSERT INTO entities_intel_reports (allegation_id, profile_id ) VALUES ('$id', '$profile_id')";
	
			$sql = "INSERT INTO entities_intel_reports (report_id, profile_id) VALUES ('$id', '$profile_id')";

	$result_sql = sqlsrv_query($conncss,$sql) or trigger_error("SQL query Failed", E_USER_ERROR);   //order executes

    header("location: intelligence_report_details.php?new_entity='yes'&tabs=#tabs-2"); 
